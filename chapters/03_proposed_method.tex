...
\begin{figure}[tb]
  \begin{center}
    \includegraphics[width=0.95\textwidth]{figures/application_overview.png}
  \end{center}
  \caption{}\label{fig:system_flow}
\end{figure}
...
このマップファイルには，各デバイスの ID と，それに対応する一意な「ラベル（Label）」が定義される。
LLM はこの「ラベル」のみを認識して操作手順を組み立てるため，
生成される API 仕様は物理的なデバイス ID（MAC アドレス等）から分離され，高い移植性を持つ。
また，マップファイル内でデバイスの動作モード（\texttt{pressMode} / \texttt{switchMode}）を指定することで，
LLM が生成すべき操作タイプ（押すだけか，ON/OFF 指定が必要か）を制約チェック時に検証することも可能となっている。
