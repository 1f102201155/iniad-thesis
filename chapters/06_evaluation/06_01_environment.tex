...
%---------- ハードウェア構成 ---------------
\subsection{ハードウェア構成}
評価実験には，Table~\ref{tab:eval-hw} に示す構成の PC を用いた。
...
本研究では SwitchBot Bot の実機を十分に用意できなかったため，
クラウド API を介して実際に家電を操作する評価は行っていない。
ただし，本研究の目的は
\textbf{API 仕様生成とコード生成が正しく行われているかを検証することであり，
家電の実動作結果は SwitchBot API やデバイス側の仕様に依存するため，
本研究の評価対象外である}。
そのため，本章では仕様・フローの整合性および生成コードの起動性に基づいて評価を行う。
